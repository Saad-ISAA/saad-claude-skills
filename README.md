...
| [`musaad`](musaad/) | `/musaad` — use **[Musaad](https://app.musaadai.com)** as Claude's working memory and task tracker through its MCP server. Roadmaps for initiatives, boards for tasks and subtasks, linked so progress rolls up; card before code, cards moved as work moves, evidence on Done, asides captured as Backlog cards, and every session resumed from the board. Includes a full tool map, call recipes and a drop-in `CLAUDE.md` block. | A Musaad account and the Musaad MCP server connected. See [skill README](musaad/README.md). |
| [`spine`](spine/) | `/spine` — keep **one implementation per concern** in codebases an AI agent writes most of. The spine pattern (registry → resolver → seam → enforcement test) across authorization, multi-tenancy, middleware, tools/MCP/integrations, configuration and credentials, event buses, notifications, jobs, databases, LLM routing, vector stores, DLP, observability and agent-to-agent delegation. Detects spines that already exist under any name, and migrates opportunistically, by joint, or in full. Includes a drift inventory, test recipes, a spine map and a drop-in `CLAUDE.md` block. | None |
| [`spine-security`](spine-security/) | `/spine-security` — every security control at **exactly one chokepoint, defended by a test**. 22 vertebrae with control ids (authentication, sessions, tenant isolation, credentials and `.env`, injection, rate limiting, headers/CORS/CSRF, crypto, SSRF, uploads, webhooks, queues, AI and MCP, supply chain, deployment exposure, incident readiness), mapped to OWASP Top 10, API Security Top 10 and LLM Top 10. Audit, change review, control consolidation, credential-leak response, a control register, and a security inventory that never prints a secret. Builds on `spine`. | None |
